...
addCheck('Env example launch vars', includesAll(envExample, ['DATABASE_URL', 'JWT_SECRET', 'WEB_URL', 'NEXT_PUBLIC_API_URL', 'NEXT_PUBLIC_BRAND_NAME', 'NEXT_PUBLIC_BRAND_LOGO_PATH', 'NEXT_PUBLIC_SUPPORT_CONTACT', 'BRAND_NAME', 'SUPPORT_CONTACT', 'EMAIL_DRY_RUN', 'BILLING_CURRENCY', 'BILLING_STARTER_PRICE_MONTHLY', 'BILLING_BUSINESS_PRICE_MONTHLY', 'BILLING_PRO_PRICE_MONTHLY', 'STRIPE_SECRET_KEY', 'STRIPE_WEBHOOK_SECRET']), '.env.example documents launch-critical variables.');
...
addCheck('Hosting env checklist', includesAll(hostingEnvChecklist, ['DATABASE_URL', 'NEXT_PUBLIC_API_URL', 'BILLING_CURRENCY=PGK', 'NEXT_PUBLIC_BRAND_LOGO_PATH', 'pnpm smoke:hosted', 'EMAIL_DRY_RUN=true']), 'Hosting checklist includes database, API, web, PGK, branding, email, and hosted smoke settings.');
